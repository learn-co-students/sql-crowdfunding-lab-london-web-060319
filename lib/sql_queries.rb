...
"Write your SQL query Here"
"SELECT (users.name)AS name, sum(pledges.amount)AS amount FROM users INNER JOIN pledges ON users.id = pledges.user_id GROUP BY users.name ORDER BY amount ,name ;"
end
